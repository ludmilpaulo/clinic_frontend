...
} from "@/services/consultationCategoryService";
...
const Availability: React.FC<AvailabilityProps> = ({ userId }) => {
  ...
  const handleEdit = (availability: any) => {
    ...
    setDaysOfWeek(
      availability.days_of_week
        ? availability.days_of_week.split(",").map(String)
        : [],
    );
    ...
    setConsultationCategory(
      categories.find(
        (category) => category.id === availability.consultation_category,
      )?.name || "",
    );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let categoryId = categories.find(
      (category) => category.name === consultationCategory,
    )?.id;

    if (!categoryId) {
      try {
        if (token) {
          const addedCategory = await createConsultationCategory(
    ...
    try {
          ...
          setAvailabilities(
            availabilities.map((avail) =>
              avail.id === editingAvailability.id
                ? { ...availabilityData, id: editingAvailability.id }
                : avail,
            ),
          );
          setEditingAvailability(null);
        } else {
          const newAvailability = await createDoctorAvailability(
            availabilityData,
            token,
          );
      ...
      if (axios.isAxiosError(error)) {
        ...
        alert(
          `Error saving availability: ${error.response?.data || error.message}`,
        );
      } else {
  ...
  const tileClassName = ({ date, view }: { date: Date; view: string }) => {
    if (view === "month") {
      ...
      const availability = availabilities.find(
        (avail) =>
          avail.days_of_week?.includes(day) ||
          avail.day_of_month === date.getDate(),
      );
      if (availability) {
        return "bg-green-200";
      }
              ...
              if (availability) {
                return (
                  <div className="text-center text-black bg-green-400 rounded p-1">
                    <p>
                      {availability.start_time} <br /> {availability.end_time}
                    </p>
                  </div>
                );
              }
          ...
          >
            <div className="text-lg font-semibold mb-2">
              ...
              {availability.day_of_month && (
                <span>/{availability.day_of_month}</span>
              )}
            </div>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-4">
                ...
                <select
                  value={dayOfMonth}
                  onChange={(e) =>
                    handleDayOfMonthChange(Number(e.target.value))
                  }
